...
def cari_regex(pattern, data):
    matched = []
    lowerPattern = word_tokenize(pattern) 
    final_pattern_1 = '[\s\S]*' + ' '.join(lowerPattern) + '[\s\S]*'
    final_pattern_2 = '[\s\S]*'.join(lowerPattern)

    match = (re.search(final_pattern_1, data) or re.search(final_pattern_2, data))
    if (match):
        return 0
    else:
        return -1
...
# Fungsi pencarian solusi
def get_solution(pattern, dataset, parameter, tipepencarian): 
    match = []
    i = 0
    stop = stopwords.words('english') + stopwords.words('indonesian')
    olahpattern = word_tokenize(pattern) 
    filtered_pattern = []
    for w in olahpattern: 
        if w.lower() not in stop: 
            filtered_pattern.append(w.lower()) 
    # filtered_pattern = [token.lower() for token in filtered_pattern]
    while i < len(dataset):
        data = dataset[i]
        olahdata = word_tokenize(data) 
        filtered_data = [] 
        for w in olahdata: 
            if w.lower() not in stop: 
                filtered_data.append(w.lower()) 
        # filtered_data = [token.lower() for token in filtered_data]

        case = -1
        if tipepencarian == 1:
            # print(" ".join(filtered_data))
            # print(" ".join(filtered_pattern))
            case = kmp((" ".join(filtered_data)), (" ".join(filtered_pattern)))
        elif tipepencarian == 2:
            case = boyer_moore((" ".join(filtered_pattern)), (" ".join(filtered_data)))
        else:
            case = cari_regex((" ".join(filtered_pattern)), (" ".join(filtered_data)))
            # regex
        if(case != -1):
            persentase = len(" ".join(filtered_pattern))/len(" ".join(filtered_data))
...
if len(pattern) == 0:
    ...
    if len(solusi) <= 0:
        ...
        #print(pertanyaan[solusi[0][0]])
        print(jawaban[solusi[0][0]])
    else:
        print("Terdapat pertanyaan-pertanyaan dibawah: ")
        print("1. " + pertanyaan[solusi[0][0]])
        print("2. " + pertanyaan[solusi[1][0]])
        if len(solusi) > 2:
            print("3. " + pertanyaan[solusi[2][0]])
